...
1.  Clone and checkout the `main` branch of the
    [Frontier Node Template](https://github.com/substrate-developer-hub/frontier-node-weavers-pay/).
...
4.  There are actually two packages in the Node Template, `frontier-node-template` (the node),
    `weaverspay-runtime` (the runtime); Each has its' own `Cargo.toml`. Inside these three
